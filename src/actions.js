...
export const searchCharacters = name => dispatch => {
  //console.log('function: searchCharacters');
  
  //Dispatch our search request to show loading.
  dispatch(searchCharactersRequest());
  
  //Fire our search and handle the responses
  search(name)
  .then(data => {
    //console.log(data);
    dispatch(searchCharactersSuccess(data));
  })
  .catch(error =>{
    //console.log(error);
